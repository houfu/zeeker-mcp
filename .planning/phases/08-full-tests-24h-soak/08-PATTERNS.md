...
**Cross-package re-export pattern:** The soak workload is the SAME data needed by `scripts/soak/run_soak.py`. Per REQUIREMENTS / single-source-of-truth: define canonical workload in `tests/_corpus/soak_workload.py`; `scripts/soak/workload.py` re-exports via `from tests._corpus.soak_workload import WORKLOAD as WORKLOAD`. Caveat: `scripts/` is not pytest-rooted, so `scripts/soak/workload.py` must adjust `sys.path` (one-line `Path(__file__).resolve().parent.parent.parent` insert). **Recommendation:** canonical in `tests/_corpus/soak_workload.py`; `scripts/soak/workload.py` re-exports. Inverted from the original "canonical in scripts/" recommendation to keep the canonical workload pytest-rooted; `scripts/` requires sys.path adjustment to import either way, and pytest collection stays clean when the canonical lives under `tests/_corpus/`. Plan 08-05 implements this direction; document the choice in the plan SUMMARY.
